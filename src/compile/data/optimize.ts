...
export class MoveParse extends BottomUpOptimizer {
  /**
   * Move parse nodes up to forks.
   */
  public moveParseUp(node: DataFlowNode): optimizers.OptimizerFlags {
   ...
   */
  public mergeParse(node: DataFlowNode): optimizers.OptimizerFlags {
    const parent = node.parent;
    if (parent === undefined) {
      return this.flags;
    }
  ...
  public optimize(node: DataFlowNode): optimizers.OptimizerFlags {
    this.flags = this.moveParseUp(node);
    this.flags = this.mergeParse(node);
...
function optimizationDataflowHelper(dataComponent: DataComponent) {
  ...
  mutatedFlag = runOptimizer(mutatedFlag, getLeaves(roots), MoveParse);
